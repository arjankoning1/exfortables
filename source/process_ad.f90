subroutine process_ad(ptype, nuclide, list)
!
!-----------------------------------------------------------------------------------------------------------------------------------
! Purpose: Process angular distributions
!
...
!
! 1. Elastic angular distribution
!
  MF = 4
  MT = 2
  ...
  close(2)
!
! 2. Inelastic and non-elastic angular distribution
!
  do MT = 4, 848
    if (MT >= 5 .and. MT < 50) cycle
    if (MT >= 90 .and. MT < 103) cycle
    if (MT >= 108 .and. MT < 600) cycle
    open (unit = 2, status = 'old', file = trim(list))
    do
      read(2,'(a9,1x,2i6,1x,a60)', iostat=istat) subentry, MF_file, MT_file, reaction
      if (istat == -1) exit
